...
import type { ExpressionBackend } from "@formbar/expressions";
import { describe, expect, it } from "vitest";
import { literal, namespace, op, ref } from "./expression-fixtures.js";

/** Host providers can reuse these public-contract fixtures without adopting Kuery's implementation. */
export function providerConformance(name: string, backend: () => ExpressionBackend): void {
	describe(name, () => {
		it.each([
			["add", 7, 3, 10],
			["subtract", 7, 3, 4],
			...
			const service = createExpressionService({ backend: backend() });
		...
		it("tracks canonical dependencies and only notifies changed projections", () => {
			...
			const service = createExpressionService({ backend: backend(), namespaces: { data: state.provider } });
			const compiled = service.compile(op("add", ref("b"), op("multiply", ref("a"), ref("a"))));
		...
		it("denies absent capabilities and separates missing from null", () => {
			const service = createExpressionService({
				backend: backend(),
				namespaces: { data: namespace({ a: null }).provider },
			});
