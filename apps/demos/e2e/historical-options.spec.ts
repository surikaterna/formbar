	...
	test(`${demo} native options in direct and playground routes`, async ({ page }) => {
		...
		for (const route of [`?demo=${demo}`, `?mode=playground&demo=${demo}&preset=default`]) {
			...
			if (demo === "array-items") {
				...
				await expect(tag.getByRole("option")).toHaveCount(3);
				...
				await expect(data).toContainText('"tags": [\n    ""');
				await preview.getByRole("button", { name: "Add Team Members" }).click();
				await preview.getByRole("textbox", { name: "Name", exact: true }).fill("Ada");
				const role = preview.getByRole("combobox", { name: "Role" });
				await expect(role.getByRole("option").first()).toHaveAttribute("value", "");
				await role.selectOption("option-3");
				await preview.getByRole("button", { name: "Add Team Members" }).click();
			...
			if (demo === "array-items") {
				const preview = page.locator("form[data-formbar-definition]");
				await expect(preview.getByRole("combobox", { name: "Tag" })).toHaveCount(0);
				await preview.getByRole("button", { name: "Add Tags" }).click();
				await expect(preview.getByRole("combobox", { name: "Tag" })).toHaveValue("option-0");
			}
		}
