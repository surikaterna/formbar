...
Here `data.amount` is input snapshot data, not a computation target. The first FSX slice evaluates conditions and outputs against a captured snapshot and renders the output only: no computed form-data mutation, computation scheduling/write-back, or hidden side effects. `computations[i].expression` remains a **future** program slot with a static dependency/validation graph obligation; executing computations and writing targets needs a separate reviewed issue before implementation.
...
| Future field `visible`/`required`, read-only prop, and `computations[i].expression` | Validate slots and static dependencies/graph; preserve node/instance fallback behavior. Computation execution/write-back is a separate reviewed issue, not first-slice acceptance. |
...
The [component registry design (#183)](https://github.com/surikaterna/formbar/issues/183) decides renderable component IDs/props separately. [Real FSX (#184)](https://github.com/surikaterna/formbar/issues/184) owns source lowering/range mapping; [writes (#180)](https://github.com/surikaterna/formbar/issues/180) owns repeated-item stable write semantics. None is implemented by this document. Outstanding signoff: confirm installed released core capabilities and client/schema/runtime release impact before treating this proposal as acceptance. The greenfield V1 in-place envelope and rejection of old Kuery slots remain the proposed breaking admission choice; `Option.none` omission is not an open product decision.
