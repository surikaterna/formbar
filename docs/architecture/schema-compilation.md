...
The interactive playground covers all 21 numbered demos (26 preset variants) and a schema-compilation example (27 examples total). Its serializable JSON editors for schema, definition, and initial data run through the production `useSchemaForm` → `FormRenderer` path with validation; executable widgets, nodes, and actions come from fixed, trusted host profiles, not editable JSON. [Trusted renderer extensions (#69)](https://github.com/surikaterna/formbar/issues/69) are implemented; [composed-schema presentation (#49)](https://github.com/surikaterna/formbar/issues/49) and [stored computations (#129)](https://github.com/surikaterna/formbar/issues/129) are not promised. #101/TUI work remains separate.
