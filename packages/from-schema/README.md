...
For the additive `submission: { hiddenValues: "omit-inactive" }` outgoing request mode, validated authored definitions must declare the same policy; generated definitions accept the option directly. `generation.submitWhenHidden` selects validated generated FieldNode IDs to include when hidden. `prepared.createForm` and `prepared.createDeferredForm` attach the definition-bound supplier and final-payload check; bare core forms do not. The default still submits the entire retained draft. Omission is not deletion from a server record. Backends must specify POST/PATCH/PUT missing-key semantics, authorize and validate writes, and account for logs, backups and retention. JSON Schema `required`/`if`/`then` and caller sync/async validators validate the **final outgoing payload**, so they may reject an omitted key. Trusted host callbacks must consume their supplied candidate, not a captured form; arbitrary same-realm closures/Proxies are not a privacy sandbox. See [React rendering and migration guidance](../react-schema/README.md#opt-in-outgoing-omission).
...
Prepared factories install JSON Schema validation and preparation-time caller validators automatically, before additional `coreOptions.validators`, followed by scoped validation. Do not pass `prepared.validators` back as core validators: duplicate prepared references (including a repeated preparation-time validator) throw instead of running twice. Independently supplied core validators retain their own order and remain unscoped. Standard Schema's separate `sourceValidator` remains a separate handle; React supplies it once via core options.
