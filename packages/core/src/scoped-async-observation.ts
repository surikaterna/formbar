...
export function observationContext(owned = false): ObservationContext {
	return { arrays: new WeakMap(), owned };
}
	...
	try {
		for (const segment of binding) {
			...
			if (Array.isArray(value)) {
				...
				if (!array) {
					array = { reference: value, snapshot: context.owned ? value : structuredClone(value), owned: context.owned };
		...
		return { binding, value: context.owned ? value : structuredClone(value), arrays, valid: value !== undefined };
	} catch {
		return { binding, value: undefined, arrays: [], valid: false };
	}
}
	...
	try {
		for (const segment of observation.binding) {
			...
			if (Array.isArray(value)) {
				...
				if (same === undefined) {
					same = array.owned || structuredEqual(array.snapshot, value);
		...
		return observation.arrays.every((array) => !array.owned)
			? structuredEqual(observation.value, value)
