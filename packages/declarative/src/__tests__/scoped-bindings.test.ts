...
describe("scoped runtime bindings", () => {
	it("resolves nested repeaters and contextual direct lifecycle without cross-item aliasing", () => {
		const { form, runtime: port } = runtime(nestedDefinition(), {
			initialData: {
				orders: [
					{ lines: [{ sku: "a", status: "" }, { sku: "b", status: "" }] },
					{ lines: [{ sku: "c", status: "" }] },
				],
			},
		});
		expect(port.getSnapshot().fields.filter((item) => item.instance.nodeId === "sku").map((item) => item.binding.segments)).toEqual([
			["orders", 0, "lines", 0, "sku"],
			["orders", 0, "lines", 1, "sku"],
			["orders", 1, "lines", 0, "sku"],
		]);
		expect(port.getSnapshot().nodes.filter((item) => item.instance.nodeId === "status").map((item) => item.visible)).toEqual([
			false,
			false,
			false,
		]);
	...
	it("rebinds deterministic instances after array replacement and removal", () => {
		...
		form.setValue("orders", [{ lines: [{ sku: "x", status: "" }, { sku: "y", status: "" }] }]);
	...
	it("exposes only direct contextual valid, validating, dirty, and touched lifecycle", () => {
		...
		expect(port.getSnapshot().nodes.filter((item) => item.instance.nodeId.startsWith("read-")).map((item) => item.visible)).toEqual([
			true,
			true,
			true,
			true,
