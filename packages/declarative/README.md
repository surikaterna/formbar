...
Output nodes are pure snapshot projections. A visible output evaluates its `value` expression against the same captured state as the rest of the runtime and exposes either its raw JSON value or a code-only failure. The default expression profile includes bounded `sumBy(collection, literalPath)` for finite numeric totals over variable-length JSON arrays; collection edits and structural replacement react through the collection expression's normal dependency. `sumBy` remains projection-only and never creates stored form data. Hidden outputs are not evaluated. The optional `label` is presentation metadata, and `format` is one of `plain`, `number`, `currency-usd`, or `percent`; formatting is renderer-owned and never changes form data, field lifecycle, validation, reset state, or submission payloads.
